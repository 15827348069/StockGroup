...
public class StockSellActivity extends ExActivity implements View.OnClickListener{
    //TODO 查询出来显示会崩溃
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_stock_sale);
    ...
    private void setupView() {
        ...
        findViewById(R.id.sale_code_id).setOnClickListener(this);
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if(s.length()>0){
                    ...
                    if(number>can_buy_number){
                        UiCommon.INSTANCE.showTip("卖出数量不能超过可卖数量");
                    }
                }
            }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.sale_code_id:
                ...
                if (TextUtils.isEmpty(buy_price.getText())) {
                    UiCommon.INSTANCE.showTip("卖出价格不能为空");
                    return;
                }
                ...
                if (TextUtils.isEmpty(buy_number.getText())
                    ...
                    UiCommon.INSTANCE.showTip("卖出数量不能为空");
                    return;
                }
                if (Double.valueOf(buy_number.getText().toString()) % 100 != 0) {
                    UiCommon.INSTANCE.showTip("卖出数量请输入100的倍数");
                    return;
                }
    ...
    private void update(final String code) {
        ...
        timer.schedule(new TimerTask() {
            ...
            public void run() {
                if (GetTime.getData()) {
                    new GetStockRealtimeInfoTask(StockSellActivity.this).execute(code);
                } else {
                    new GetStockRealtimeInfoTask(StockSellActivity.this).execute(code);//最后一次更新
                    if (timer != null) {
                        timer.cancel();
                        timer = null;
                        return;
    ...
    private class GetStockRealtimeInfoTask extends LoadingDialog<String, Stock> {
        ...
        public Stock doInBackground(String... params) {
            ...
            Stock stock = null;
            try {
                stock = server.getRealtimeInfo(mb.getId(),params[0]);
        ...
        public void doStuffWithResult(Stock result) {
            if (result != null && result.code != -1) {
                ...
                UiCommon.INSTANCE.showTip(StockSellActivity.this.getString(R.string.load_fail));
            }
        }
    }
    ...
    private class BuyTask extends LoadingDialog<String, General> {
        ...
        public General doInBackground(String... params) {
            ...
            try {
                return server.sell(buy_code, buy_price_msg, buy_number_msg, mb.getId());
            } catch (JSONException e) {
                e.printStackTrace();
            }
            return null;
...
//                    initData();
    ...
    private void initData() {
        buy_price.setText("");
        can_buy_number_text.setText("可卖:" + "--");
        buy_number.setText("");
