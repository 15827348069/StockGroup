...
import com.zbmf.StockGroup.beans.DownloadInfo;
import com.zbmf.StockGroup.beans.UploadInfo;
...
public class DataSet {
	...
	public static void init(Context context){
		sqLiteOpenHelper = new SQLiteOpenHelper(context,context.getPackageName(), null, 1) {
			@Override
			...
			synchronized (downloadInfoMap) {
				...
				for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
					try {
						...
						LogUtil.e("Parse date error"+e.getMessage());
					}
				}
			}
		} catch (Exception e) {
			LogUtil.e("cursor error"+e.getMessage());
		} finally{
			if (cursor != null) {
				cursor.close();
			}
