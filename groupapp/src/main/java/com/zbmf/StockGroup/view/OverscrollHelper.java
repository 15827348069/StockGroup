...
import com.zbmf.StockGroup.view.PullToRefreshBase.Mode;
import com.zbmf.StockGroup.view.PullToRefreshBase.State;
...
public final class OverscrollHelper {
    ...
    private static OverscrollHelper mOverscrollHelper=null;
    public static OverscrollHelper getmOverScrollHelper(){
    ...
    private static ScrollViewChangeListener scrollViewChangeListener = null;
    /**
     * 最小的滑动距离
     */
    private static final int SCROLL_LIMIT_Y = 10;
    public void setScrollViewChangeListener(ScrollViewChangeListener scViewChangeListener){
        scrollViewChangeListener=scViewChangeListener;
    }
    /**
     * Helper method for Overscrolling that encapsulates all of the necessary
        ...
        if (view.isPullToRefreshOverScrollEnabled() && !view.isRefreshing()) {
            ...
            if (mode.permitsPullToRefresh() && !isTouchEvent && deltaValue != 0) {
                ...
                if (scrollViewChangeListener!=null){
                    scrollViewChangeListener.onScroll(deltaValue+scrollValue,newScrollValue);
                }
                if (scrollY<newScrollValue&&deltaY>=SCROLL_LIMIT_Y){
                    if (scrollViewChangeListener!=null){
                        scrollViewChangeListener.scrollTop();
                    }
                }else if (scrollY>newScrollValue&&-deltaY>=SCROLL_LIMIT_Y){
                    if (scrollViewChangeListener!=null){
                        scrollViewChangeListener.scrollDown();
                    }
                }

                /*if (deltaValue>=0){//往上滑动
                    if (scrollViewChangeListener!=null){
                        scrollViewChangeListener.topToShow();
                    }
                }else {
                    //往下滑动
                    if (scrollViewChangeListener!=null){
                        scrollViewChangeListener.bottomToShow();
                    }
                }*/
